...
async function generateMeshReplicate(prompt, params = {}) {
    ...
    const response = await axios.post(url, { version, input }, {
        headers: {
...
server.registerTool("generate_3d_mesh", {
    ...
    inputSchema: {
        ...
        output_path: z
            .string()
            .optional()
            .describe("Path to save the OBJ mesh file (e.g., my_mesh.obj). If omitted, a .obj file will be created automatically."),
        guidance_scale: z
            .number()
            .optional()
            .describe("Guidance scale for Shap-E (higher = more adherence to prompt). Default 15."),
        batch_size: z
            .number()
            .optional()
        ...
        render_size: z
            .number()
            .optional()
            .describe("Render resolution; also affects mesh quality. Default 128."),
        save_mesh: z
            .boolean()
            .optional()
            .describe("Whether to save mesh data instead of only GIF renders. Default true."),
    },
}, async ({ prompt, output_path, guidance_scale, batch_size, render_mode, render_size, save_mesh, }) => {
    try {
